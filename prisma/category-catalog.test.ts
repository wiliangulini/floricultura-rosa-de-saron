...
describe("categoryCatalog", () => {
  it("contém exatamente as sete categorias com os dados definidos em description.md", () => {
    expect(categoryCatalog).toEqual(expectedCatalog);
  });

  it("mantém ordens sequenciais e slugs sem duplicidade", () => {
    const sortOrders = categoryCatalog.map((category) => category.sortOrder);
    const slugs = categoryCatalog.map((category) => category.slug);

    expect(sortOrders).toEqual([1, 2, 3, 4, 5, 6, 7]);
    expect(new Set(sortOrders).size).toBe(categoryCatalog.length);
    expect(new Set(slugs).size).toBe(categoryCatalog.length);
  });
});
